...
$t$ and sending $u$ to $u^\lambda$, and $\pi_2^\ast\colon R\to R_2$ for the face
...
\begin{theorem}[Proposition~3.5: triangularization over an algebraically closed field]
  ...
  Let $I$ be a Novikov isocrystal over $K$.  There are a natural number $n$, an
...
\begin{proof}
  ...
  Lemma~\ref{lem:recover_dd_from_isoc} writes $x=\pi_1^\ast(y)$ for some
...
\begin{proof}
  ...
  $\varphi(\pi_1^\ast m)\in\pi_2^\ast M$ are fixed by $F_{M,2}$, and they generate
  $\pi_2^\ast M$ after extension of scalars.  In the coordinates defining $T$, the
...
\begin{lemma}[Removing a rank-one extension]
  ...
  \[
    x_i=\sum_j\pi_2^\ast(A_{ij})F_2(x_j)
      +\pi_2^\ast(C_i)F_2(\xi).
  \]
...
\begin{proof}
  ...
  calculation gives
  $E(\pi_2^\ast(a)y)=aE(y)$, $E\circ F_2=F\circ E$, and
  $E\circ\pi_2^\ast=\mathrm{id}$.  Applying $E$ coordinatewise and setting
...
\begin{proof}
  ...
  \[
    y\longmapsto y_{\mathrm{top}}-\pi_2^\ast(Z)y_n.
  \]
